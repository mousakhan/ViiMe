...
class RedemptionViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, CLLocationManagerDelegate {
    ...
    override func viewDidLoad() {
        ...
        let numberOfPeople = Int(self.deal?.numberOfPeople ?? "0") ?? 0
        self.numberOfPeopleLabel.text = "Bring out \(numberOfPeople - 1) friends to redeem this deal!"
        
        // Create the message to show
        var subTitle = "\(self.deal?.shortDescription ?? "") \n\n Deal valid until \(DateHelper.parseDate(date: self.deal?.validTo ?? ""))"
        
        if (self.deal?.validFrom == "" || self.deal?.validTo == "") {
            subTitle = self.deal?.shortDescription ?? ""
        }
        
        
        let recurringTo = DateHelper.parseTime(time: self.deal?.recurringTo ?? "")
        let recurringFrom = DateHelper.parseTime(time: self.deal?.recurringFrom ?? "")
...
//        return (self.group?.users.count  ?? 0) + 1
        return Int(self.deal?.numberOfPeople ?? "0") ?? 0 + 1
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if (indexPath.row == 0) {
            ...
            cell.profilePicture.image = UIImage(named: "empty_profile")
            cell.nameLabel.text = "?"
            cell.nameLabel.textColor = FlatYellowDark()
        }
...
//        }
        ...
        redemptionTextField.keyboardType = .numberPad

        
        alertView.addButton("Redeem", backgroundColor: FlatPurple(), action: {
            if (CLLocationManager.locationServicesEnabled() && redemptionTextField.text == self.venue?.code) {
                switch(CLLocationManager.authorizationStatus()) {
                case .notDetermined, .restricted, .denied:
                    BannerHelper.showBanner(title: "Location services must be enabled to redeem deal", type: .danger)
                ...
                case .authorizedAlways, .authorizedWhenInUse:
                    ...
                    // Set value on the group redemption object
                    Constants.refs.redemptions.childByAutoId().setValue(["title": self.deal?.title ?? "", "short-description": self.deal?.shortDescription ?? "", "num-people": self.deal?.numberOfPeople ?? "", "valid-from": self.deal?.validFrom ?? "", "valid-to": self.deal?.validTo ?? "", "recurring-from": self.deal?.recurringFrom ?? "", "recurring-to": self.deal?.recurringTo ?? "", "num-redemptions": self.deal?.numberOfRedemptions ?? "", "active": false, "latitude": self.currentLocation.latitude, "longitude": self.currentLocation.longitude,  "redeemed": ServerValue.timestamp(), "owner": self.user?.username ?? "", "venue": self.venue?.id ?? ""
                        ])
        ...
        alertView.showInfo("Enter redemption code", subTitle: "Please pass it to the merchant to enter the redemption code")
        redemptionTextField.becomeFirstResponder()
    }
